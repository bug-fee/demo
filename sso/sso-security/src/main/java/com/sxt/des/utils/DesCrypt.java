...
public class DesCrypt {
    ...
    public static String encode(String key, String datasource) throws Exception{
        ...
        //使用IOUtils.toString方法，将UTF-8数据转换为1个字节对应一个字符的ISO-8859-1或16进制数。保证数据的定长
    ...
    public static String decode(String key, String src) throws Exception {
        ...
        // DES算法要求有一个可信任的随机数源.支持第一个参数如果种子相同，则生成的随机信息可推测。
        SecureRandom random = new SecureRandom();
        // 创建一个DESKeySpec对象。依据提供的密匙字符串创建密匙。密钥源信息。需要通过密钥工厂再次推算的，才能得到最终的密钥数据。
        DESKeySpec desKey = new DESKeySpec(key.getBytes(CODE_TYPE));
        // 创建一个密匙工厂，然后用它把DESKeySpec转换为SecretKey
        SecretKeyFactory keyFactory = SecretKeyFactory.getInstance("DES");
        ...
        // 真正开始解密操作
        //使用base64进行额外编码解码的目的是保证数据长度的可靠性（避免数据过长）

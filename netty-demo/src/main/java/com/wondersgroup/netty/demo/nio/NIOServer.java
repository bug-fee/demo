...
public class NIOServer implements Runnable {
    ...
    //业务开发中不在全局定义这两个缓存对象，原因是该全局对象非线程安全的对象。
    ...
    private void init(int port) {
        try {
            ...
            System.out.println("sver started");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void run() {
        while (true) {
            try {
                ...
                //通道是否选中，由注册到多路复用器中的通道标记决定的（注册时声明状态 serverChannel.register(this.selector, SelectionKey.OP_ACCEPT) ）
                this.selector.select();
                //返回以选中的通道标记集合，集合中保存的是通道的标记。相当于是通道的ID
                Iterator<SelectionKey> keys = this.selector.selectedKeys().iterator();
                while (keys.hasNext()) {
                    SelectionKey key = keys.next();
                    //将本次要处理的通道从集合中删除，下次循环根据新的通道列表再次执行必要的业务处理
                    keys.remove();
                    //通道是否有效
                    if (key.isValid()) {
                        //阻塞状态
                        try {
                            //判断状态是否等于可连接 SelectionKey.OP_ACCEPT
                            if (key.isAcceptable()) {
                                accept(key);
                            }
    ...
    private void read(SelectionKey key) {
        try {
            ...
            int readLenth = channel.read(readBuffer);
            //检查客户端是否写入数据。若没有则关闭通道
            if (readLenth == -1) {
                //关闭通道
                key.channel().close();
                //关闭连接
